...
export const appSchema = yup.object().shape({
	...
	discord: yup
		.string()
		.matches(/^.{3,32}(#[0-9]{1,5})?$/giu, 'Invalid discord tag')
		.max(37, 'Character limit exceeded')
		.required('Required'),
	btag: yup
		.string()
		.matches(/^.{3,32}#[0-9]{4,6}$/giu, 'Invalid battle tag')
		.max(20, 'Character limit exceeded'),
	charName: yup
		.string()
	...
	wclLink: yup
		.string()
		.matches(
			/^(http:\/\/www\.|https:\/\/www\.|http:\/\/|https:\/\/)?(classic\.)?(warcraftlogs)\.(com)(\/.*)?$/giu,
			'Invalid warcraftlogs url'
		)
	...
	vouch: yup.string().max(150, 'Character limit exceeded'),
	historyRaids: yup
		.string()
	...
	historyGuilds: yup
		...
		.max(1023, 'Character limit exceeded')
		.required('Required'),
	extraInfo: yup.string().max(1023, 'Character limit exceeded'),
});
